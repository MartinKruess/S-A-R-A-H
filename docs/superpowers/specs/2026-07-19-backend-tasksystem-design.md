# S.A.R.A.H. — Backend / Hintergrundaufgaben-System (Design)

**Datum:** 2026-07-19 (ausgelagert aus `problems/features.md`).
**Status:** Design/Plan für **viel später** — noch nicht terminiert. Umgesetzt ist bislang nur **§11 Lückenfüller V1** (PR #25, gesprochene Swap-Bridge `frontendThinking`/`switchingBack`). Der Rest — Frontend/Backend-Router-Trennung, persistente Task-Queue, Worker-Loop, Ergebnis-Events, die übrigen Füllsatz-/Statusmeldungs-Kategorien — wartet auf die Umsetzung dieses Systems.

> Vor der Umsetzung eigenes Brainstorm→Spec→Plan. Verwandte offene Punkte (Langzeit-Kontext, generische Tool-Calls, Wake-Word) stehen in `problems/features.md`.
