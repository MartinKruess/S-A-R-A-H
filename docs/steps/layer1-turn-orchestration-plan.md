...
| 150 | P2 | Ein erwarteter F9-Abbruch des Worker-Streams wurde als `Output job failed: AbortError` protokolliert und verfälschte die Laufzeitdiagnose trotz fachlich erfolgreichem Cancel. |
| 151 | P2 | VoiceService terminalisierte den unterbrochenen Vorgängerturn bereits, während RouterService nach dem Abort ein zweites Terminalereignis erzeugte. MessageBus verweigerte dieses als unbekannten Turn und meldete eine irreführende Lifecycle-Warnung. |
| 152 | P1 | Ein F9-Abbruch stellte den Router sofort wieder her und entlud dadurch das bereits geladene 8B-Modell. Eine direkte Folgefrage musste den vollständigen Modellwechsel erneut durchlaufen, statt das normale Fünf-Minuten-Warmhaltefenster zu nutzen. |
| 153 | P1 | Beim F9-Abbruch nach begonnenem Worker-Stream löschte RouterService den gesamten Turn-Draft. Dadurch verschwand auch die sichtbare Nutzerfrage aus dem Modellkontext; eine unspezifische Folgefrage wie „Erzähl mir mehr dazu“ bezog sich fälschlich auf den davor abgeschlossenen Turn. |
...
- Der reale Chat-/Speak-A/B-Test bestand denselben Worker-Fallback in beiden Modi. Chat zeigte genau eine vollständige Textantwort; Speak ergänzte nur den vorgesehenen Wartezeitüberbrücker und eine inhaltlich identische vollständige Sprachausgabe.
- Die Befunde 150 und 151 wurden behoben: Erwartete Stream-Abbrüche werden still als Queue-Fortsetzung behandelt, und extern bereits terminalisierte Turns werden zentral erinnert, sodass kein zweites Terminalereignis mehr entsteht.
- Befund 152 wurde behoben: Ein abgebrochener, bereits erreichter Worker bleibt bis zum normalen Idle-Restore warm; echte Fehler sowie Abbrüche während des Modellwechsels stellen den Router weiterhin unmittelbar wieder her.
- Befund 153 wurde behoben: Nach tatsächlich begonnenem Worker-Output bleibt ausschließlich die Nutzerfrage im flüchtigen Live-Kontext. Teilantworten werden verworfen und nie persistiert; Abbrüche vor dem ersten Chunk, Action-/Routing-Abbrüche und Shutdowns übernehmen keinen unvollständigen Turn. Persistierter Startkontext akzeptiert weiterhin nur vollständige Turns.
- Die praktische F9-Wiederholung bestand: Die unfertige Bubble verschwand vollständig, `qwen3:8b` blieb mit 78 % GPU / 22 % CPU geladen, es entstanden keine Abort-/Terminalwarnungen, und „Erzähl mir mehr dazu“ bezog sich korrekt auf die zuvor abgebrochene Betriebssystemfrage statt auf den älteren Himmel-Turn.
- Bewusst zurückgestellte UX-Beobachtung, kein Layer-1-Befund: Während einer langen gestreamten Chatantwort folgt die kleine Chatansicht weiter dem neuen Text und beeinflusst damit manuelles Hochscrollen. Eine spätere Verbesserung soll nur automatisch nach unten folgen, solange der Nutzer bereits am unteren Rand steht; für die aktuelle Abnahme wurde ausdrücklich keine Änderung gewünscht.
- Bewusst zurückgestellte Command-UX, kein Layer-1-Befund: Ein späteres `/help` kann verfügbare Befehle auflisten. Bei ähnlich geschriebenen unbekannten Commands darf Sarah höchstens „Meintest du …?“ vorschlagen; insbesondere `/anonymous`, `/confirm` und andere Commands dürfen niemals per unscharfer Erkennung automatisch ausgeführt werden.
- Bewusst zurückgestellte Cockpit-Visualisierung, kein Layer-1-Befund: Die zentrale Kugel soll bei Sarahs Sprachausgabe später deutlicher audio-reaktiv wabern. Eine mögliche Ablösung durch die geplante holografische 2,5D-Darstellung wird gemeinsam mit deren GPU-/CPU-Kosten erst im UI-/Performance-Scope bewertet.
- Die Mehrfachturn-Prüfung bestand zwei unmittelbar gesendete Chatturns mit getrennten Processing-Bubbles, getrennten IDs, FIFO-Ausführung und genau je einer Antwort ohne Vermischung oder Doppelung.
- Die Slash-Command-Matrix bestand unbekannte, bekannte noch nicht verfügbare und ungültig parametrierte verfügbare Commands ohne Modellaufruf; `/anonymous` wurde produktiv beantwortet und bleibt technisch als flüchtiger Turn abgesichert.
- Der Router-/Action-Wechsel bestand Fehler- und Erfolgspfad: Ein nicht freigegebener Taschenrechner wurde ehrlich abgelehnt, das freigegebene AppX-Ziel Spotify wurde gestartet und ein anschließender Wissens-Turn wechselte kontrolliert zurück zum Worker.
- Die Multi-Window-Prüfung bestand Hauptfenster, Cockpit und Einstellungen mit weiterlaufendem Worker-Turn und ohne Renderer-Fan-out-Warnung beim Schließen eines Zusatzfensters.
- Die kontrollierte Whisper-Recovery bestand einen erzwungenen Prozessabsturz: Sarah startete `large-v3-turbo` selbstständig neu, erreichte wieder HTTP 200 und verarbeitete anschließend F9, Transkription, Worker und TTS ohne App-Neustart.
- Die kontrollierte Ollama-Recovery bestand einen gestoppten `sarah-ollama`-Container: Der laufende Voice-Turn erhielt eine ehrliche Worker-Degradationsantwort, der Container und Router wurden automatisch wiederhergestellt und der anschließende Chatturn lud den Worker und antwortete normal.
- Der abschließende Shutdown bestand erneut: Whisper bestätigte `/shutdown` mit HTTP 200, Electron und Sarahs Python-Prozess endeten vollständig, kein Modell blieb geladen, der leere Ollama-Container durfte weiterlaufen und das extern gestartete Spotify blieb unberührt.
- Der integrierte Stand bestand 90 Testdateien mit 979 Tests, Main- und Renderer-Typecheck, den vollständigen Produktionsbuild und `git diff --check`. Die native SQLite-Bindung wurde anschließend wieder für Electron 41.1.1 hergestellt.
- Die aktualisierte praktische Windows-Matrix ist für den aktuellen Layer-1-/Layer-0-Scope abgeschlossen. Künftige Layer-2- und höhere Integrationen werden gezielt gegen diese Verträge regressionsgeprüft, ohne automatisch ein weiteres vollständiges Layer-1-Audit auszulösen.
