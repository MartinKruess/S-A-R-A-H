...
export class SarahHexOrb {
  ...
  constructor(
    ...
    this.innerIdleColor = this.shellColor.clone().multiplyScalar(0.94);
  ...
  private createShell() {
    const points = this.generateFibonacciSphere(380, this.radius);
  ...
  public triggerBreak(durationMs = 2000) {
  ...
  private animate = () => {
    ...
    if (this.innerSphere && this.innerMaterial) {
      ...
      this.innerMaterial.emissiveIntensity = 0.025 + activeGlow * 0.38;
    ...
    for (let i = 0; i < this.segments.length; i++) {
      ...
      const radialOffset = envelope * (2.81 + microWobble);
