...
import { MediaContext, MEDIA_CONTEXT_WINDOW_MS } from '../../../src/services/llm/media-context.js';

const CONTEXT_TURN_ID = 'media-context-turn';

describe('MediaContext.resolve', () => {
  it('"weiter" after a pause resumes (media_play)', () => {
    const c = new MediaContext();
    c.record('media_pause', 1000, CONTEXT_TURN_ID);
    expect(c.resolve('weiter', 2000)).toEqual({
      action: 'media_play',
      speak: 'Läuft wieder.',
      contextTurnId: CONTEXT_TURN_ID,
    });
  });

  it('"weiter" after a skip goes to the next track (media_next)', () => {
    const c = new MediaContext();
    c.record('media_next', 1000, CONTEXT_TURN_ID);
    expect(c.resolve('weiter', 2000)).toEqual({
      action: 'media_next',
      speak: 'Nächstes Lied.',
      contextTurnId: CONTEXT_TURN_ID,
  ...
  it('"nächstes" is always media_next, even right after a pause', () => {
    const c = new MediaContext();
    c.record('media_pause', 1000, CONTEXT_TURN_ID);
    expect(c.resolve('nächstes', 2000)).toEqual({
      action: 'media_next',
      speak: 'Nächstes Lied.',
      contextTurnId: CONTEXT_TURN_ID,
    });
  ...
  it('record refreshes the sliding window', () => {
    const c = new MediaContext();
    c.record('media_next', 0, CONTEXT_TURN_ID);
    const first = c.resolve('weiter', 10_000);      // 10s < 12s → warm
    expect(first?.action).toBe('media_next');
    c.record(first!.action, 10_000, first!.contextTurnId); // refresh at 10s (RouterService does this)
