...
export class SpotifyActions {
  ...
  async setVolume(percent: number): Promise<LaunchResult> {
    ...
    return this.putVolume(token, Math.round(percent));
  }

  /** Relative volume ("Spotify leiser" → -25): read current, clamp, then set. */
  async adjustVolume(delta: number): Promise<LaunchResult> {
    const token = await this.oauth.getAccessToken('spotify');
    if (token === null) return NOT_CONNECTED;
    ...
    try {
      ...
      return this.putVolume(token, target);
    } catch (err) {
  ...
  private async putVolume(token: string, percent: number): Promise<LaunchResult> {
    try {
